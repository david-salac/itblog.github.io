...
lead = """<p class="lead">One of the common challenges when dealing with big multidimensional data sets is how effectively store them. There are many problems to deal with, but maybe the most important is choosing the proper dimension order. Although typically, the selection of the optimal dimensional order does not impact the size of the output file (if you use some compression situation that may differ), it has a profound effect on the response time when accessing stored data.</p>
...
<p>As mentioned above, data are technically stored in just one dimension. This raises the question of how it is possible to store multidimensional data to just one dimension and reconstruct them? The answer is simpler than one might think. Suppose you have just three dimensions of sizes (<span class="math"><em>l</em><sub>1</sub>, <em>l</em><sub>2</sub>, <em>l</em><sub>3</sub></span><!-- LATEX $l_1, l_2, l_3$ LATEX -->). Then each point <em class="math">P</em> <!-- LATEX $P$ LATEX --> has coordinates (<span class="math"><em>p</em><sub>1</sub>, <em>p</em><sub>2</sub>, <em>p</em><sub>3</sub></span><!-- LATEX $p_1, p_2, p_3$ LATEX -->) - always indexed from 0 to <span class="math"><em>l</em><sub>i</sub></span><!-- LATEX $l_i$ LATEX --> (right excluded). You can easily map these three coordinates to just one coordinate, say <em class="math">q</em><!-- LATEX $q$ LATEX -->, by using the relation:</p>
<p class="center math">
    <em>q</em> = <em>p</em><sub>1</sub> + <em>l</em><sub>1</sub> &#8901; <em>p</em><sub>2</sub> + <em>l</em><sub>1</sub> &#8901; <em>l</em><sub>2</sub> &#8901; <em>p</em><sub>3</sub>
</p>
<!-- LATEX 
$$q = p_1 + l_1 p_2 + l_1  l_2  p_3 $$
 LATEX -->
...
<!-- LATEX 
$$p_3 = \\left\\lfloor \\frac{q}{l_1 l_2} \\right\\rfloor$$
$$p_2 = \\left\\lfloor \\frac{q - p_3 l_1 l_2}{l_1} \\right\\rfloor$$
$$p_1 = q - p_3 l_1 l_2 - p_2 l_1 $$
 LATEX -->
<p>where <span class="math">FLOOR</span><!-- LATEX $\\left\\lfloor \cdots \\right\\rfloor$ LATEX --> is the floor function (take just the integer part, ignore the decimal one).</p>
