...
lead = """Library GDAL (developed by OSGeo) is more or less standard for the processing of geospatial data. However, although it implements almost all the tools needed for this purpose, it is notoriously known for many operational problems. Mainly, it is practically impossible to install it quickly (as it requires many system-level dependencies), API is obsolete (designed in the late nineties)."""

content = """Library GDAL (developed by OSGeo) is more or less standard for the processing of geospatial data. However, although it implements almost all the tools needed for this purpose, it is notoriously known for many operational problems. Mainly, it is practically impossible to install it quickly (as it requires many system-level dependencies), API is obsolete (designed in the late nineties), and it is not difficult to find unexpected behaviours, etc. So it is not surprising that many developers started to seek different tools actively. This post presents some possibilities for the most common issues (mainly focused on Python users).

<h2>Reading GeoTIFF files </h2>
<p>GeoTIFF is still a popular standard. It is mainly used for storing raster layers (like map tiles). GDAL naturally provides functionality for reading GeoTIFF files - but it is traditionally hard to use. Typically GeoTIFF file follows some logic - data inside are correctly georeferenced and are stored in one band. If this is the case, the easiest way how to read such a file is to use a library called rasterstats (https://pypi.org/project/rasterstats/). It provides a simple way to read data from a GeoTIFF file using various geometry definitions (using WTK, GeoJSON, etc.) and many ways to process that data (compute mean of data inside geometry maximal, minimal values and many others). An example of such code can follow the logic:</p>
...
<p>There are a few applications of GDAL where there is no other sufficient product available. One of the very common applications is geospatial coordinates transformation (for example, from EPSG:4326 to EPSG:3856). There are some ways how to circumvent this issue using the underpinning library of QGIS - but this treatment is maybe worse than a disease (as installing QGIS is in many ways more difficult than installing GDAL). For these specific purposes, GDAL is a very helpful tool. Also, it is good to be aware that many libraries mentioned above use GDAL internally (it is just intelligently wrapped, so you can use install a library in a user-friendly way).</p> 
<p>Another library worth mentioning is Fiona - even though it uses GDAL internally, it can be easily installed using PIP. It is beneficial for the creation of custom shapefiles and the processing of shapefiles generally. Moreover, Fiona is performance-optimized (working faster than library GeoPandas which can also be used in some limited way for this purposes).</p>

<h2>Tools using GDAL internally</h2>
<p>Many tools use GDAL internally. The most typical example is GeoServer - it is a trendy tool for providing map tiles (and layers) developed again by OSGeo. Unfortunately, it has many disadvantages. Similarly to GDAL, the quality of code is deficient. In addition, the application is written as a single server monolithic application - definitely not a small service that is simple to scale (scaling is generally very difficult). Also, recovery processes often files, which makes GeoServer challenging to use.</p>
