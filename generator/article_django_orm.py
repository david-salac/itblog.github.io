...
lead = """One of the traditional arguments on why to avoid using ORM is the lack of performance. Although it is generally speaking correct, there are many ways how to write queries in your application. Some of them are better than others. There is almost always a way to significantly increase the performance of an application without avoiding the ORM approach. We anticipate some preliminary knowledge of Django and SQL here (so make sure you understand basic concepts before continuing)."""
...
<p>Say that we have some imaginary table storing information about people (day of registration, day of birth, the colour of hair). If we want to select some non-blond people born before 1990 who registered themselves before their 30<sup>th</sup> birthday, the query can look like this:</p>
...
<p>This example is an academic one. But it shows how useful these classes are. It can save you a lot of database hits compared to the naive approach when you check these conditions in the for-loop.</p>
...
<p>The two approaches above can make almost every query faster. But they are not sufficient if the logic of the application is incorrect. Unfortunately, there is no simple manual on how to design application logic in the best way. It is always dependant on your particular application. It is always necessary to design tables and selection logic to follow all the principles mentioned above (fetch all that is needed with interacting to the minimal number of tables) in one query.</p> 
