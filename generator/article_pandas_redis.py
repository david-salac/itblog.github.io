...
content = """In-memory database REDIS provides a simple interface for caching various data types. In addition, it supports simple manipulations with arrays and scalar values, including support for complex data constructs (like queues). What is, however not implemented, is the support of multi-dimensional objects. These objects are beneficial if you need to cache frequently used constants (or interim results) for computations (like matrices). There, however, exist a simple way to circumvent this obstacle.
...
<p>
There is no significant delay in the overall process up to one million entities. On selected technology, this is also an edge for the optimal size of the stored tables. Furthermore, it means that a user experience is not affected by the long time needed for receiving values below this value.
</p>
